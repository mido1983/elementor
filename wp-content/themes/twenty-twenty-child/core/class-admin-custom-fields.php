<?php
    ...
    class ACF
    {
        function __construct()
        {
            ...
            {
                add_meta_box("product_meta", " Details", "add_details_product_meta_box", "product", "normal", "low");
            }
            
            function add_details_product_meta_box()
            {
                global $post;
                $custom = get_post_custom($post->ID);
                $value = get_post_meta($post->ID, 'product_is_on_sale', true);
                ?>
                <style>
                    .acf-row {
                        display: flex;
                ...
                <div class="container ">
                    <div class="form-group">
                        <label for="product_price"><?php _e('Price:', 'pmg'); ?></label>
                        <input type="text" class="form-control" id="product_price" name="product_price" value="<?= @$custom["product_price"][0] ?>"/>
                    </div>
                    <div class="form-group">
                        <label for="product_sale_price"><?php _e('Sale price:', 'pmg'); ?> </label>
                        <input type="text" class="form-control" id="product_sale_price" name="product_sale_price" value="<?= @$custom["product_sale_price"][0] ?>"/>
                    </div>
                    <div class="form-check-inline">
                        <label class="form-check-label">
                            <input type="checkbox" class="form-check-input" value="1" <?php checked($value, true, true); ?> name="product_is_on_sale"> <?php _e('Product on sale? (Yes/No)', 'pmg'); ?>
                            <br><br>
                        </label>
                    </div>
                    <div class="form-group">
                        <label for="product_video_link"><?php _e('Video link: ', 'pmg'); ?> </label>
                        <input type="text" class="form-control" id="product_video_link" name="product_video_link" value="<?= @$custom["product_video_link"][0] ?>"/>
                    </div>
                    <div class="form-group">
                        <label for="acf_product_description"><?php _e('Description: ', 'pmg'); ?></label>
                        <input type="textarea" class="form-control" cols="100%" rows="7" id="acf_product_description" name="acf_product_description"
                               value="<?= @$custom["acf_product_description"][0] ?>"> </input>
                    </div>
                    <hr>
                    <dl class="item-property">
                        <dt>ShortCode params</dt>
                    </dl>
                    <div class="form-group">
                        <label for="acf_product_background_color"> <?php _e('Select background color:', 'pmg'); ?></label>
                        <input type="color" id="acf_product_background_color" name="acf_product_background_color" value="<?= @$custom["acf_product_background_color"][0] ?>">
                    </div>
                    <div class="form-group">
                        <label for="acf_product_shortcode_name"> <?php _e('Shortcode:', 'pmg'); ?></label>
                        <input type="text" class="form-control" id="acf_product_shortcode_name" name="acf_product_shortcode_name" value="<?= @$custom["acf_product_shortcode_name"][0] ?>"/>
                    </div>
                </div>
            ...
            function save_product_custom_fields()
            {
                ...
                if ($post) {
                    ...
                    update_post_meta($post->ID, "product_sale_price", @$_POST["product_sale_price"]);
                    update_post_meta($post->ID, "product_is_on_sale", @$_POST["product_is_on_sale"]);
                    update_post_meta($post->ID, "product_video_link", @$_POST["product_video_link"]);
                    update_post_meta($post->ID, "acf_product_description", @$_POST["acf_product_description"]);
                    update_post_meta($post->ID, "acf_product_background_color", @$_POST["acf_product_background_color"]);
                    update_post_meta($post->ID, "acf_product_shortcode_name", @$_POST["acf_product_shortcode_name"]);
                }
                $test = new Custom_shortcode();
                $atts = array(
                    'id' => $post->ID,
                    'bg-color' => $_POST["acf_product_background_color"],
                    'name' => $_POST["acf_product_shortcode_name"],
                );
                $test->shortcode($atts, 'kkk');
            }
